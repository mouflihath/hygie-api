...
<div class="root">
    ...
    <div class="page-header">
        <div>
            ...
            <p>Commissions collectées sur la plateforme Hygie+</p>
        </div>
    ...
    <div class="grid-2">
        ...
        <div class="card">
            <div class="card-header">
                <div class="card-title"><div class="card-dot" style="background:#F59E0B"></div>Top Pharmacies</div>
                <span class="card-badge">Par commission</span>
            </div>
            <div style="overflow-x:auto">
                <table class="dash-table">
                    ...
                    <tbody>
                        @forelse($parPharmacie as $i => $row)
                        @php
                            $rankColors = ['#F59E0B','#94A3B8','#CD7F32'];
                            $rc = $rankColors[$i] ?? '#EEF1F7';
                        @endphp
                        <tr>
                            <td>
                                <span class="rank-badge" style="background:{{ $rc }}22;color:{{ $rc }}">
                                    {{ $i + 1 }}
                                </span>
                            </td>
                            <td>
                                <div class="pharma-name">{{ $row->pharmacie->nom_pharmacie ?? 'Pharmacie #'.$row->pharmacie_id }}</div>
                            </td>
                            <td style="text-align:right">
                                <span class="amount-gray">{{ $row->nb_commandes }} cmd</span>
                            </td>
                            <td style="text-align:right">
                                <span class="amount-green">{{ number_format($row->total, 0, ',', ' ') }} F</span>
                            </td>
                        </tr>
                        @empty
                        <tr><td colspan="4" style="padding:40px;text-align:center;color:#C8D0DC;font-size:.72rem;font-weight:700;text-transform:uppercase;">Aucune donnée</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
